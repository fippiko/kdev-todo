...
public class ManageProjectsViewDesktop extends Composite implements
		ManageProjectsView {
	private static ManageProjectsDesktopUiBinder uiBinder = GWT
			.create(ManageProjectsDesktopUiBinder.class);

	interface ManageProjectsDesktopUiBinder extends
			UiBinder<Widget, ManageProjectsViewDesktop> {
	}
	...
	private void reloadProjectList() {
		...
		/*
		 * projectList.clear(); for (ProjectProxy project :getProjects allProjects) {
	...
	void editProjectButtonClicked(ClickEvent e) {
		String selectedProject = projectList.getValue(projectList
				.getSelectedIndex());
